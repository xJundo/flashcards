...
/** Public: reading a folder's banner needs no account, same as the folder itself. */
export async function GET(_request: Request, { params }: Params) {
  const { id } = await params
  const banner = await getFolderBanner(id)
  if (!banner)
    return NextResponse.json(
      { error: "Aucune bannière pour ce dossier." },
      { status: 404 }
    )
...
/** Replaces whatever banner the folder already had — there is only ever one. */
export async function POST(request: Request, { params }: Params) {
  ...
  await saveFolderBanner(id, { contentType, data, size: data.byteLength })
